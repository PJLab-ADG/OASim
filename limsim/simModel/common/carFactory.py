...
class Vehicle:
    ...
    def getLaneLevelRoute(self, nb: NetworkBuild) -> tuple[set, dict]:
        ...
        if lastEdgeID not in LLRDict.keys():
            LLRDict[lastEdgeID] = {
    ...
    def plotSelf(self, vtag: str, widget: QWidget, ex: float, ey: float, ctf: CoordTF):
        if not self.xQ:
            raise TypeError('Please call Model.updateVeh() at first.')
        rotateMat = np.array(
            [
        ...
        if vtag == 'ego':
            vcolor = QColor(211, 84, 0)
        elif vtag == 'AoI':
            vcolor = QColor(41, 128, 185)
        else:
            vcolor = QColor(99, 110, 114)
        widget.drawPolygon(drawVex, vcolor, vcolor)
        # graphics_items.append(graphicsItem('polygon', drawVex, vcolor, vcolor))
        # signal.emit('polygon', drawVex, vcolor, vcolor, 1, False, "")

    def plotTrajectory(self, widget: QWidget, ex: float, ey: float, ctf: CoordTF):
        if self.plannedTrajectory and self.plannedTrajectory.xQueue:
            tps = [
                ctf.qtCoord(
            ...
            widget.drawPolyline(tps, QColor(205, 132, 241))
            # graphics_item.append(graphicsItem(
            #     'polyline', tps, QColor(205, 132, 241), QColor(205, 132, 241)))
            # signal.emit('polyline', tps, QColor(205,132,241), QColor(205,132,241), 1, False, "")

    def plotDBTrajectory(self, widget: QWidget, ex: float, ey: float, ctf: CoordTF):
        if self.dbTrajectory and self.dbTrajectory.xQueue:
            ...
            widget.drawPolyline(tps, QColor(255, 112, 85), 2)
            # graphics_item.append(graphicsItem(
            #     'polyline', tps, QColor(225, 112, 85), QColor(225, 112, 85), 2))
            # signal.emit('polyline', tps, QColor(225, 112, 85), QColor(225, 112, 85), 2, False, "")
...
class egoCar(Vehicle):
    def __init__(
        self, id: str, deArea: float = 100, sceMargin: float = 20
    ) -> None:
        super().__init__(id)
        # detection area
        self.deArea = deArea
    ...
    def plotdeArea(self, widget: QWidget, ex: float, ey: float, ctf: CoordTF):
        shape = [ctf.qtCoord(self.x, self.y, ex, ey), QPointF(ctf.zoomScale*self.deArea *
                            2, ctf.zoomScale*self.deArea*2)]
        widget.drawEllipse(shape, QColor(255, 255, 255), QColor(243, 204, 153, 30))
        # graphics_item.append(graphicsItem('ellipse', shape, QColor(
        #     255, 255, 255), QColor(243, 204, 153, 30)))
...
class DummyVehicle:
    ...
    def plotArea(self, node: dpg.node, ex: float, ey: float, ctf: CoordTF):
        cx, cy = ctf.qtCoord(self.x, self.y, ex, ey)
        dpg.draw_circle(
            (cx, cy),
