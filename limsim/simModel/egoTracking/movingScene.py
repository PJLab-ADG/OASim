...
class MovingScene:
    ...
    def plotScene(self, draw_items: list, ex:float, ey:float, ctf: CoordTF):
        if self.edges:
            for ed in self.edges:
                self.netInfo.plotEdge(ed, draw_items, ex, ey, ctf)

        if self.junctions:
            for jc in self.junctions:
                self.netInfo.plotJunction(jc, draw_items, ex, ey, ctf)
...
class SceneReplay:
    ...
    def plotScene(self, graphics_items: list, ex: float, ey: float, ctf: CoordTF):
        if self.edges:
            for ed in self.edges:
                self.netInfo.plotEdge(ed, graphics_items, ex, ey, ctf)

        if self.junctions:
            for jc in self.junctions:
                self.netInfo.plotJunction(jc, graphics_items, ex, ey, ctf)