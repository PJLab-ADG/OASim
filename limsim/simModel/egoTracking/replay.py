...
from hmi.routeEdit.gui import GUI, GUI_QT
...
class ReplayModel:
    ...
    def drawSce(self):
        ...
        self.sr.plotScene(self.gui_qt.window, ex, ey, self.gui_qt.ctf)
        self.ego.plotSelf('ego', self.gui_qt.window, ex, ey, self.gui_qt.ctf)
        self.ego.plotdeArea(self.gui_qt.window, ex, ey, self.gui_qt.ctf)
        self.ego.plotDBTrajectory(self.gui_qt.window, ex, ey, self.gui_qt.ctf)
        if self.sr.vehINAoI:
            for vAoI in self.sr.vehINAoI.values():
                vAoI.plotSelf('AoI', self.gui_qt.window,
                              ex, ey, self.gui_qt.ctf)
                vAoI.plotDBTrajectory(self.gui_qt.window,
                                      ex, ey, self.gui_qt.ctf)
        if self.sr.outOfAoI:
            for vSce in self.sr.outOfAoI.values():
                vSce.plotSelf('outOfAoI', self.gui_qt.window,
                              ex, ey, self.gui_qt.ctf)
    ...
    def getSce(self):
        ...
        if nextFrameVehs:
            ...
            if not self.tpEnd:
                ...
                self.tpEnd = 1
                self.gui_qt.is_running = 0

    def render(self):
        self.gui_qt.update_inertial_zoom()
        self.gui_qt.window.clearAll()
        ...
        self.drawSce()
        # self.plotVState()
        self.gui_qt.window.update()
        # dpg.render_dearpygui_frame()
        if self.gui_qt.replayDelay:
            time.sleep(self.gui_qt.replayDelay)

    @ property
    def canGetNextSce(self) -> int:
        if self.gui_qt.is_running:
            return 1  # just move steps
        else:
            if self.frameIncrement < self.gui_qt.frameIncre:
    ...
    def moveStep(self):
        ...
        if not self.gui_qt.is_running:
            self.tpEnd = 1
        self.render()
        sys.exit(self.gui_qt.app.exec())
